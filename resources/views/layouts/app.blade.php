...
.nav-link::after { content:''; position:absolute; bottom:-4px; left:0; right:0; height:1.5px; background:#C8965A; transform:scaleX(0); transition:transform .25s; }
.nav-link:hover::after, .nav-link.active::after { transform:scaleX(1); }
#toast { position:fixed; bottom:2rem; right:2rem; background:#0D1F3C; color:#fff; padding:.8rem 1.4rem; border-radius:4px; font-size:.82rem; z-index:9999; display:none; border-left:3px solid #C8965A; }
...
<!-- NAVBAR -->
<header class="sticky top-0 z-50 bg-[#0D1F3C] border-b border-white/10">
  <div class="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
    ...
    <!-- RIGHT -->
    <div class="flex items-center gap-4">
      <a href="{{ route('home', ['lang'=> app()->getLocale()=='id' ? 'en' : 'id']) }}" class="hidden sm:flex items-center gap-1.5 text-[10px] font-bold tracking-widest text-white/60 hover:text-[#C8965A] transition uppercase">
      ...
      <a href="{{ route('cart.index') }}" class="relative" id="cart-icon">
        <svg class="w-5 h-5 text-white/80 hover:text-[#C8965A] transition" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
        @php $cartCount = collect(session('cart', []))->sum('qty'); @endphp
        @if($cartCount > 0)
        <span id="cart-badge" style="position:absolute;top:-8px;right:-8px;background:#C8965A;color:white;font-size:10px;width:17px;height:17px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700">{{ $cartCount }}</span>
        @endif
      </a>
  ...
  <div id="mobile-menu" class="md:hidden bg-[#0A1828] border-t border-white/10 px-6 py-6">
    <nav class="flex flex-col gap-6">
      <a href="{{ route('home') }}" class="text-sm tracking-widest uppercase text-white/80 hover:text-[#C8965A] transition" onclick="document.getElementById('mobile-menu').classList.remove('open')">Beranda</a>
      <a href="{{ route('philosophy') }}" class="text-sm tracking-widest uppercase text-white/80 hover:text-[#C8965A] transition" onclick="document.getElementById('mobile-menu').classList.remove('open')">Filosofi</a>
      <a href="{{ route('product') }}" class="text-sm tracking-widest uppercase text-white/80 hover:text-[#C8965A] transition" onclick="document.getElementById('mobile-menu').classList.remove('open')">Produk</a>
...
<!-- FOOTER -->
<footer class="bg-[#070F1C] text-white pt-16 pb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-12 pb-12 border-b border-white/10">
      <div>
        ...
        <div class="flex gap-3">
          <span class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white/40 hover:border-[#C8965A] hover:text-[#C8965A] transition cursor-pointer text-xs">in</span>
          <span class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white/40 hover:border-[#C8965A] hover:text-[#C8965A] transition cursor-pointer text-xs">ig</span>
          <span class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white/40 hover:border-[#C8965A] hover:text-[#C8965A] transition cursor-pointer text-xs">wa</span>
        </div>
      </div>
      <div>
        <h4 class="text-[10px] tracking-[.25em] uppercase text-[#C8965A] mb-5 font-semibold">{{ __('footer.nav_title') }}</h4>
        <ul class="space-y-3">
      ...
      <div>
        <h4 class="text-[10px] tracking-[.25em] uppercase text-[#C8965A] mb-5 font-semibold">{{ __('footer.info_title') }}</h4>
        <ul class="space-y-3">
...
document.addEventListener('click', e => {
  ...
  fetch('{{ route('cart.add') }}', {
    ...
    if (badge) { badge.textContent = res.count; }
      ...
      badge.id = 'cart-badge';
      badge.style.cssText = 'position:absolute;top:-8px;right:-8px;background:#C8965A;color:white;font-size:10px;width:17px;height:17px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700';
