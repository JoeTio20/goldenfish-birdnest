...
@section('head')
<style>.serif{font-family:'Cormorant Garamond',serif}.thumb{border:2px solid transparent}.thumb.active{border-color:#C9A84C}@media(max-width:767px){.detail-grid{grid-template-columns:1fr!important}.related-grid{grid-template-columns:1fr!important}}@media(min-width:768px) and (max-width:1023px){.related-grid{grid-template-columns:repeat(2,1fr)!important}}</style>
@endsection
@section('content')
<section style="background:#F5F8F6;padding:42px 24px 72px"><div style="max-width:1120px;margin:0 auto">
 <a href="<?php echo e(route('product')); ?>" style="font-size:12px;color:#4A6B6B;text-decoration:none">&larr; <?php echo e(__('messages.back_to_products')); ?></a>
 <div class="detail-grid" style="display:grid;grid-template-columns:1.05fr .95fr;gap:38px;align-items:start;margin-top:24px">
  ...
  <div style="background:#fff;border:1px solid rgba(200,168,76,.12);border-radius:18px;padding:28px">
   ...
   <form method="POST" action="<?php echo e(route('cart.add')); ?>">@csrf<input type="hidden" name="product_id" value="<?php echo e($product->id); ?>"><button type="submit" <?php echo $product->stock <= 0 ? 'disabled' : ''; ?> style="width:100%;padding:14px;font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;background:<?php echo $product->stock <= 0 ? '#9ca3af' : '#C9A84C'; ?>;color:#0D3535;border:none;border-radius:10px;cursor:<?php echo $product->stock <= 0 ? 'not-allowed' : 'pointer'; ?>"><?php echo e($product->stock <= 0 ? __('messages.stock_empty') : __('messages.add_cart')); ?></button></form>
  </div>
 </div>
 @if($related->count())<h2 class="serif" style="font-size:28px;color:#1A3D3A;margin:48px 0 18px"><?php echo e(__('messages.related_products')); ?></h2><div class="related-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px">@foreach($related as $p)<a href="<?php echo e(route('product.show',$p)); ?>" style="background:#fff;border-radius:14px;overflow:hidden;border:1px solid rgba(200,168,76,.12);text-decoration:none;color:inherit"><img src="<?php echo e($p->thumbnail); ?>" style="width:100%;aspect-ratio:4/3;object-fit:cover" onerror="this.src='/IMAGE/SUPER.jpeg'"><div style="padding:14px"><b><?php echo e($p->name); ?></b><p style="color:#C9A84C;margin-top:4px">Rp <?php echo e(number_format($p->price,0,',','.')); ?></p></div></a>@endforeach</div>@endif
 @if(isset($recentlyViewed) && $recentlyViewed->count())<h2 class="serif" style="font-size:28px;color:#1A3D3A;margin:48px 0 18px">Recently Viewed</h2><div class="related-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:18px">@foreach($recentlyViewed as $p)<a href="<?php echo e(route('product.show',$p)); ?>" style="background:#fff;border-radius:14px;overflow:hidden;border:1px solid rgba(200,168,76,.12);text-decoration:none;color:inherit"><img loading="lazy" decoding="async" src="<?php echo e($p->thumbnail); ?>" style="width:100%;aspect-ratio:4/3;object-fit:cover" onerror="this.src='/IMAGE/SUPER.jpeg'"><div style="padding:14px"><b><?php echo e($p->name); ?></b><p style="color:#C9A84C;margin-top:4px">Rp <?php echo e(number_format($p->price,0,',','.')); ?></p></div></a>@endforeach</div>@endif
