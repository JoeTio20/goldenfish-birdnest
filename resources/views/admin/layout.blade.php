...
<style>
*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}body{background:#1B3A1F;font-family:'Inter',sans-serif;margin:0;color:#E8E0D4;overscroll-behavior:none;}.serif{font-family:'Playfair Display',serif;}#desktop-sidebar{background:#0A1A0A;border-right:1px solid rgba(255,255,255,.07);}.nav-item{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;font-size:12.5px;color:rgba(255,255,255,.45);cursor:pointer;text-decoration:none;transition:all .15s;margin:2px 8px;}.nav-item:hover{background:rgba(200,150,90,.12);color:#C9A84C;}.nav-item.active{background:rgba(200,150,90,.18);color:#C9A84C;border-left:2px solid #C9A84C;padding-left:12px;font-weight:600;}.nav-item svg{width:16px;height:16px;flex-shrink:0;}#bottom-nav{display:none;}@media(max-width:767px){#bottom-nav{display:flex;position:fixed;bottom:0;left:0;right:0;background:#0A1A0A;border-top:1px solid rgba(255,255,255,.07);z-index:50;padding-bottom:env(safe-area-inset-bottom);}.bnav-item{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:9px 4px 11px;gap:3px;text-decoration:none;color:rgba(255,255,255,.35);font-size:9px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;transition:color .15s;}.bnav-item.active,.bnav-item:hover{color:#C9A84C;}.bnav-item svg{width:20px;height:20px;}#main-content{padding-bottom:70px;}#desktop-sidebar{display:none!important;}#mobile-topbar{display:flex!important;}}@media(min-width:768px){#mobile-topbar{display:none;}#desktop-sidebar{display:flex;}}#mobile-topbar{display:none;align-items:center;justify-content:space-between;padding:12px 16px;background:#0A1A0A;border-bottom:1px solid rgba(255,255,255,.07);position:sticky;top:0;z-index:40;}.card{background:rgba(255,255,255,.04);border-radius:14px;padding:20px;border:1px solid rgba(255,255,255,.07);}
</style>
</head>
<body>
...
<div id="mobile-topbar">
  <div class="flex items-center gap-2">
    <div class="w-7 h-7 rounded-lg flex items-center justify-center" style="background:#C9A84C">
  ...
  <div class="flex items-center gap-3">
    ...
    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style="background:#C9A84C;color:#1B3A1F">
...
<aside id="desktop-sidebar" class="hidden md:flex flex-col" style="width:230px;min-height:100vh;flex-shrink:0;">
  ...
  <div class="py-4" style="border-top:1px solid rgba(255,255,255,.07)">
    <div class="flex items-center gap-3 px-4 py-3 mx-3 rounded-xl" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07)">
      <div class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0" style="background:#C9A84C;color:#1B3A1F">
        {o} substr(auth('admin')->user()->name ?? 'A', 0, 1) {c}
      </div>
...
<!-- MAIN -->
<div class="flex-1 flex flex-col min-w-0" id="main-content" style="background:#0F1A0F">
  <header class="hidden md:flex items-center justify-between px-8 py-4" style="background:#0A1A0A;border-bottom:1px solid rgba(255,255,255,.07)">
...
function showToast(msg,type='success') {L}
  ...
  if(!t) {L} t=document.createElement('div');t.id='admin-toast';t.style.cssText='position:fixed;top:4.5rem;right:1rem;padding:.7rem 1.2rem;border-radius:8px;font-size:.8rem;z-index:9999;display:none;font-weight:500;border-left:3px solid #C9A84C';document.body.appendChild(t); {R}
  t.style.background=type==='success'?'rgba(10,26,10,.97)':'rgba(220,38,38,.9)';
