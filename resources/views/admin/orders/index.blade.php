...
@if(session('success'))
<div class="text-sm px-4 py-3 rounded-xl mb-4" style="background:rgba(201,168,76,.15);border:1px solid rgba(201,168,76,.3);color:#6BAED6">{{ session('success') }}</div>
@endif
<div class="rounded-2xl overflow-hidden" style="background:rgba(255,255,255,.03);border:1px solid rgba(245,237,216,.08)">
  <table class="w-full text-sm">
    ...
    <tbody>
      ...
      @php
        $statusMap = ['pending'=>['Pending','rgba(201,168,76,.15)','#6BAED6'],'dikonfirmasi'=>['Dikonfirmasi','rgba(96,165,250,.15)','#60a5fa'],'dikemas'=>['Dikemas','rgba(192,132,252,.15)','#c084fc'],'diantar'=>['Diantar','rgba(99,102,241,.15)','#818cf8'],'selesai'=>['Selesai','rgba(74,222,128,.15)','#4ade80'],'dibatalkan'=>['Dibatalkan','rgba(248,113,113,.15)','#f87171']];
        $s = $statusMap[$order->status] ?? ['Pending','rgba(201,168,76,.15)','#6BAED6'];
      @endphp
      <tr class="hover:bg-white/5 transition" style="border-bottom:1px solid rgba(245,237,216,.05)">
        ...
        <td class="px-4 py-3 font-medium" style="color:#D8E4ED">{{ $order->full_name }}</td>
        <td class="px-4 py-3 font-semibold hidden md:table-cell" style="color:#6BAED6">Rp {{ number_format($order->total,0,',','.') }}</td>
        <td class="px-4 py-3">
        ...
        <td class="px-4 py-3">
          <a href="{{ route('admin.orders.show', $order) }}" class="text-xs font-semibold hover:underline" style="color:#6BAED6">Detail &rarr;</a>
        </td>
      </tr>
      @empty
