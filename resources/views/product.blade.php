...
<section style="background:#F5F8F6;padding:18px 24px;border-bottom:1px solid rgba(200,168,76,.1)">
 <div style="max-width:1140px;margin:0 auto;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px" class="filter-bar">
  ...
  <div style="display:flex;align-items:center;gap:8px">
   ...
   <select onchange="location='{{ route('product') }}?category={{ request('category','all') }}&sort='+this.value" style="font-size:12px;padding:7px 12px;border-radius:8px;border:1.5px solid rgba(200,168,76,.25);background:#fff;color:#1A3D3A;outline:none;cursor:pointer">
...
<section style="background:#F5F8F6;padding:36px 24px 72px">
 ...
 <div style="margin-bottom:20px;padding:12px 16px;border-radius:10px;background:rgba(200,168,76,.08);border:1px solid rgba(200,168,76,.2);color:#C9A84C;font-size:13px">{{ session('cart_success') }}</div>
 @endif
 <div class="prod-grid">
 ...
 <div class="prod-card">
  ...
  <div class="prod-card-body">
   ...
   <p style="font-size:12px;color:#4A6B6B;margin-bottom:8px;line-height:1.6">{{ Str::limit($p->description,65) }}</p>
   <p style="font-size:14px;font-weight:700;color:#C9A84C;margin-bottom:0">Rp {{ number_format($p->price,0,',','.') }}</p>
   <div class="prod-card-spacer"></div>
   <form method="POST" action="{{ route('cart.add') }}" style="margin-top:14px">@csrf
   <input type="hidden" name="product_id" value="{{ $p->id }}">
   <button type="submit" style="width:100%;padding:11px;font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;background:#C9A84C;color:#0D3535;border:none;border-radius:8px;cursor:pointer" onmouseover="this.style.opacity=.8" onmouseout="this.style.opacity=1">@lang('messages.add_cart')</button>
   </form>
  </div>
 </div>
 @empty
 <div style="grid-column:1/-1;text-align:center;padding:80px 0;color:#4A6B6B;font-size:14px">Belum ada produk tersedia.</div>
 @endforelse
 </div>
 </div>
</section>
