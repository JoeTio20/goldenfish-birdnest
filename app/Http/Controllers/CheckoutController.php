...
class CheckoutController extends Controller
{
    ...
    private function generateOrderNumber(): string { $prefix='GBN-'.now()->format('Ymd').'-'; $count=Order::whereDate('created_at',now()->toDateString())->count()+1; do { $number=$prefix.str_pad($count,4,'0',STR_PAD_LEFT); $count++; } while(Order::where('order_number',$number)->exists()); return $number; }
    public function store(Request $request)
    {
        $request->validate(['first_name'=>'required|string|max:100','last_name'=>'required|string|max:100','email'=>'required|email|max:150','whatsapp'=>'required|string|max:20','address'=>'required|string','city'=>'required|string','postal_code'=>'nullable|string|max:10','payment_method'=>'required|in:transfer,midtrans']);
    ...
    private function sendOrderEmail(Order $order): void
    {
        ...
        try{
            ...
            Mail::raw($body, fn($m)=>$m->to($order->email)->subject('Pesanan Goldenfish Birdnest #'.$order->order_number));
